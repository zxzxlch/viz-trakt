...
class Show extends Component {
  ...
  render() {
    const seasons = this.props.seasons.map(season => {
      const seasonProps = _pick(season,
    ...
    return (
      <div className={showClassName}>
        <div className="header">
          <h1 className={showTitleClassName}>{this.props.title} ({this.props.year})</h1>
          <div className={ratingBoxClassName}>
            <div className="ratingBarContainer">
              <RatingBar {...this.props.ratings} />
            </div>
            <div className="rating" title={`${this.props.ratings.votes} ratings`}>{this.props.ratings.rating.toFixed(1)}</div>
          </div>
        </div>
